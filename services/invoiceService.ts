...
import { signInvoiceMetadata, buildInvoiceMetadata, METADATA_VERSION } from "@/lib/invoiceMetadata";
  ...
  async createInvoice(
    ...
    try {
      ...
      let metadata = buildInvoiceMetadata({
        ...
        external_url: `${env.NEXT_PUBLIC_IPFS_GATEWAY}/${docCid}`,
      });

      // Sign the metadata if signMessage is provided
      if (signMessage) {
        metadata = await signInvoiceMetadata(metadata, ownerAddress, signMessage);
      }
...
export async function prepareCreateInvoice(
  ...
  const result = await service.createInvoice(formData, ownerAddress, onProgress, signMessage);
  if (!result.ok) throw new Error(result.error.message);
  return result.value;
}
