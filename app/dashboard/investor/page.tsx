  ...
  const STATS = [
    {
      ...
      change: t("stats.activePositionsChange", { count: positionsData.length }),
      changePositive: true,
      icon: <DollarSign className="h-4 w-4" />,
    },
    {
      label: t("stats.expectedYield"),
      value: formatCurrency(totalYield, "USDC", true),
      change: totalInvested > 0 ? t("stats.returnChange", { percent: ((totalYield / totalInvested) * 100).toFixed(1) }) : t("stats.returnChange", { percent: "0.0" }),
      changePositive: true,
    ...
    {
      label: t("stats.avgApr"),
      value: `${averageApr.toFixed(1)}%`,
      change: t("stats.acrossPositions"),
      changePositive: true,
  ...
  const POSITION_COLUMNS: ColumnDef<InvestorPosition>[] = [
    {
      ...
      cell: (row) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{row.invoice?.metadata.category ?? t("unspecified")}</p>
        </div>
      ),
    },
    {
      ...
      accessor: (row) => row.invoice?.metadata.debtorName ?? t("unknownDebtor"),
      cell: (row) => <span className="text-muted-foreground">{row.invoice?.metadata.debtorName ?? t("unknownDebtor")}</span>,
    },
    {
      id: "invested",
      header: t("table.invested"),
      accessor: (row) => row.investedAmount,
      cell: (row) => <span className="font-medium text-foreground">{formatCurrency(row.investedAmount, "USDC", true)}</span>,
    },
    {
      id: "expected",
      header: t("table.expectedReturn"),
      accessor: (row) => row.expectedReturn,
      cell: (row) => <span className="font-medium text-success">{formatCurrency(row.expectedReturn, "USDC", true)}</span>,
    },
    {
      id: "yield",
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          {row.status === "repaid" ? (
            <Button size="sm" onClick={() => handleClaim(row)}>{t("table.claim")}</Button>
          ) : null}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link href="/marketplace">
          <Button variant="outline">
            <Store className="h-4 w-4" /> {t("browseMarketplace")}
          </Button>
        </Link>
      </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>{t("allocationRisk")}</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("allocationJurisdiction")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {Object.entries(
              positionsData.reduce<Record<string, number>>((acc, position) => {
