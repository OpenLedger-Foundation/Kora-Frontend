...
// Mock lib/env.ts to avoid environment variable validation
vi.mock("@/lib/env", () => ({
  env: {
    NEXT_PUBLIC_STELLAR_RPC_URL: "https://rpc.testnet.stellar.org",
    NEXT_PUBLIC_STELLAR_HORIZON_URL: "https://horizon-testnet.stellar.org",
    NEXT_PUBLIC_STELLAR_NETWORK_PASSPHRASE: "Test SDF Network ; September 2015",
    NEXT_PUBLIC_INVOICE_CONTRACT_ID: "CA3D5KRYM6CB7OWQ6TWYCB6ZZ7OZ6E5A6A5GQ6A5A5GQ6A5GQ6A5A5G",
    NEXT_PUBLIC_MARKETPLACE_CONTRACT_ID: "CA3D5KRYM6CB7OWQ6TWYCB6ZZ7OZ6E5A6A5GQ6A5A5GQ6A5GQ6A5A5G",
    NEXT_PUBLIC_TOKEN_CONTRACT_ID: "CA3D5KRYM6CB7OWQ6TWYCB6ZZ7OZ6E5A6A5GQ6A5A5GQ6A5GQ6A5A5G",
...
// ─── 9. Signed Metadata Attestation ──────────────────────────────────────────────

describe("Signed Metadata Attestation", () => {
  it("signInvoiceMetadata adds attestation with signer, signature, and timestamp", async () => {
    const keypair = StellarSdk.Keypair.random();
    const signer = keypair.publicKey();
    const signMessage = async (message: string) => {
      const messageBuffer = Buffer.from(message);
      return keypair.sign(messageBuffer).toString("base64");
    };

    const signed = await signInvoiceMetadata(VALID_METADATA, signer, signMessage);
    expect(signed.attestation).toBeDefined();
    expect(signed.attestation?.signer).toBe(signer);
    expect(signed.attestation?.signature).toBeDefined();
    expect(typeof signed.attestation?.timestamp).toBe("number");
  });

  it("verifyInvoiceAttestation returns true for valid attestation", async () => {
    const keypair = StellarSdk.Keypair.random();
    const signer = keypair.publicKey();
    const signMessage = async (message: string) => {
      const messageBuffer = Buffer.from(message);
    ...
    const signed = await signInvoiceMetadata(VALID_METADATA, signer, signMessage);
    const isValid = verifyInvoiceAttestation(signed);
    expect(isValid).toBe(true);
  });

  it("verifyInvoiceAttestation returns false when no attestation", () => {
    const isValid = verifyInvoiceAttestation(VALID_METADATA);
    expect(isValid).toBe(false);
  });

  it("verifyInvoiceAttestation returns false for invalid signature", async () => {
    const validKeypair = StellarSdk.Keypair.random();
    const invalidKeypair = StellarSdk.Keypair.random();
    const signer = validKeypair.publicKey();
    const signMessage = async (message: string) => {
      const messageBuffer = Buffer.from(message);
      return invalidKeypair.sign(messageBuffer).toString("base64");
    };
  ...
  it("verifyInvoiceAttestation returns false when metadata is altered", async () => {
    const keypair = StellarSdk.Keypair.random();
    const signer = keypair.publicKey();
    const signMessage = async (message: string) => {
      const messageBuffer = Buffer.from(message);
    ...
    const signed = await signInvoiceMetadata(VALID_METADATA, signer, signMessage);
    const altered = { ...signed, amount: 999999 };
    const isValid = verifyInvoiceAttestation(altered);
    expect(isValid).toBe(false);
  });

  it("canonicalizeMetadata ignores attestation field when signing", async () => {
    const keypair = StellarSdk.Keypair.random();
    const signer = keypair.publicKey();
    const signMessage = async (message: string) => {
      const messageBuffer = Buffer.from(message);
    ...
    const signed1 = await signInvoiceMetadata(VALID_METADATA, signer, signMessage);
    const signed2 = await signInvoiceMetadata(signed1, signer, signMessage);
    const isValid = verifyInvoiceAttestation(signed2);
    expect(isValid).toBe(true);
  });
});
