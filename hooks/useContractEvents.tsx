...
type EventMessages = {
  invoiceFunded: string;
  invoiceFundedDesc: (amount: string, tokenId: string) => string;
  invoiceRepaid: string;
  invoiceRepaidDesc: (tokenId: string) => string;
  invoiceCancelled: string;
  invoiceCancelledDesc: (tokenId: string) => string;
};

function showEventToast(event: ContractEvent, walletAddress: string, msgs: EventMessages) {
  const isRelevant =
  ...
  switch (event.type) {
    case "invoice_funded":
      toast.success(
        <div className="flex flex-col gap-0.5">
          <span className="font-semibold text-foreground">{msgs.invoiceFunded}</span>
          <span className="text-xs text-muted-foreground">
            {msgs.invoiceFundedDesc(amountStr, event.tokenId)}
          </span>
        </div>,
        { duration: 5000 }
      );
    ...
    case "invoice_repaid":
      toast.success(
        <div className="flex flex-col gap-0.5">
          <span className="font-semibold text-foreground">{msgs.invoiceRepaid}</span>
          <span className="text-xs text-muted-foreground">
            {msgs.invoiceRepaidDesc(event.tokenId)}
          </span>
        </div>,
        { duration: 5000 }
      );
    ...
    case "invoice_cancelled":
      toast.info(
        <div className="flex flex-col gap-0.5">
          <span className="font-semibold text-foreground">{msgs.invoiceCancelled}</span>
          <span className="text-xs text-muted-foreground">
            {msgs.invoiceCancelledDesc(event.tokenId)}
          </span>
        </div>,
        { duration: 5000 }
      );
...
export function useContractEvents(options: UseContractEventsOptions = {}) {
  ...
  const eventMsgs: EventMessages = {
    invoiceFunded: t("invoiceFunded"),
    invoiceFundedDesc: (amount, tokenId) => t("invoiceFundedDesc", { amount, tokenId }),
    invoiceRepaid: t("invoiceRepaid"),
    invoiceRepaidDesc: (tokenId) => t("invoiceRepaidDesc", { tokenId }),
    invoiceCancelled: t("invoiceCancelled"),
    invoiceCancelledDesc: (tokenId) => t("invoiceCancelledDesc", { tokenId }),
  };
    ...
    try {
      ...
      for (const event of newEvents) {
        ...
        if (walletAddress && notificationPreferences.invoiceFunded) {
          showEventToast(event, walletAddress, eventMsgs);
        }
      }
