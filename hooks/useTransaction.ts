...
export function useTransaction() {
  ...
  const tTxSimulation = useTranslations("txSimulation");
  ...
  const pollWithBackoff = useCallback(async (hash: string): Promise<string> => {
    const deadline = Date.now() + POLL_TIMEOUT_MS;

    for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
      if (Date.now() > deadline) throw new Error(t("timeoutAfter5Minutes"));

      const result = await rpc.getTransaction(hash);
      if (result.status === "SUCCESS") return hash;
      if (result.status === "FAILED") throw new Error(t("failedOnChain"));

      const delay = Math.min(1000 * 2 ** attempt, 16_000);
      await new Promise((r) => setTimeout(r, delay));
    }

    throw new Error(t("notConfirmedAfterAttempts", { attempts: MAX_POLL_ATTEMPTS }));
  }, [t]);
  ...
  const execute = useCallback(
      ...
      try {
        ...
        if (!unsignedXdr.startsWith("mock_")) {
          ...
          const timeoutPromise = new Promise<never>((_, reject) =>
            setTimeout(
              () => reject(new Error(tTxSimulation("timeout"))),
          ...
          if (StellarSdk.rpc.Api.isSimulationError(sim)) {
            ...
            throw new Error(t("simulationFailed", { error: sim.error }));
          }
        ...
        if (signedXdr.startsWith("mock_")) {
          ...
          if (result.status === "ERROR") throw new Error(t("submissionFailed"));
          hash = result.hash;
        }
    ...
    [signTransaction, setStage, setTxState, addTransaction, updateTransactionStatus, t, tTxSimulation, toast, state.txHash, pollWithBackoff]
  );
