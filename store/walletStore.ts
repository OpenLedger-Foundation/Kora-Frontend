  ...
  persist(
    (set, get) => ({
      ...
      setVerified: (isVerified, verifiedAt) =>
        set({ isVerified, verifiedAt: verifiedAt || (Date.now() + 60 * 60 * 1000) }),
      ...
      isVerificationExpired: () => {
        ...
        // verifiedAt holds the absolute expiry timestamp (expiresAt from server)
        return Date.now() > state.verifiedAt;
      },
