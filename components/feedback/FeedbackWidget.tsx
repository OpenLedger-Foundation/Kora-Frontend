...
export function FeedbackWidget() {
  ...
  // Build schema inside component so validation messages are translated
  const feedbackSchema = z.object({
    type: z.enum(["bug", "feature", "other"]),
    title: z.string().min(3, t("validationTitleMin")).max(120),
    description: z.string().min(10, t("validationDescMin")).max(2000),
  });

  const FEEDBACK_TYPES = [
    { value: "bug" as const, label: t("typeBug"), icon: Bug, color: "text-destructive" },
    { value: "feature" as const, label: t("typeFeature"), icon: Lightbulb, color: "text-amber-400" },
    { value: "other" as const, label: t("typeOther"), icon: MessageCircle, color: "text-primary" },
  ];
  ...
  const onSubmit = useCallback(
    async (values: FeedbackFormValues) => {
      ...
      try {
        ...
        toast.error(t("errorSubmit"), { description: (err as Error).message });
      } finally {
        setSubmitting(false);
      }
    },
      ...
      <div className="fixed bottom-6 right-6 z-[9000] flex flex-col items-end gap-3">
            ...
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4" noValidate>
                ...
                <div>
                  ...
                  {screenshot ? (
                    <div className="relative rounded-lg overflow-hidden border border-border">
                      ...
                      <img src={screenshot} alt={t("screenshotAlt")} className="w-full max-h-32 object-cover" />
                      <button
                        type="button"
                ...
                <p className="text-[11px] text-muted-foreground">
                  {walletAddress ? t("contextNoteWithWallet") : t("contextNote")}
                </p>

                {/* Actions */}
                <div className="flex gap-2 pt-1">
                  <Button type="button" variant="outline" size="sm" className="flex-1" onClick={handleClose} disabled={submitting}>
                    {t("cancel")}
                  </Button>
                  <Button type="submit" variant="primary" size="sm" className="flex-1" isLoading={submitting}>
                    {t("submit")}
                  </Button>
                </div>
              </form>
        ...
        <motion.button
          ...
          className={cn(
            ...
            "sm:px-4 sm:py-2.5 max-sm:px-3 max-sm:py-3"
          )}
        ...
        >
          ...
          <span className="hidden sm:inline">{open ? t("close") : t("toggle")}</span>
        </motion.button>
      </div>
    </>
  );
