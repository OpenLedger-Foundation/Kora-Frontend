...
import { verifyInvoiceAttestation } from "@/lib/invoiceMetadata";
...
export function InvoiceMetadataViewer({ invoice, isFunded = false }: InvoiceMetadataViewerProps) {
  ...
  useEffect(() => {
    ...
    if (metadata) {
      // Check attestation
      const isAttestationValid = verifyInvoiceAttestation(metadata as InvoiceMetadataV1);
      if (!cancelled) setAttestationVerified(isAttestationValid);
    }
    ...
    <div className="space-y-6">
      ...
      <div className={cn(
        ...
        <div className="flex flex-col gap-2 items-end">
          ...
          <Badge
            variant={attestationVerified ? "info" : "warning"}
            className="h-fit"
          >
            {attestationVerified ? "Signed by Owner" : "Not Attested"}
          </Badge>
        </div>
      </div>
